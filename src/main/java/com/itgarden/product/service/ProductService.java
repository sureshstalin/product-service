...
import com.itgarden.product.dto.ProductResponse;
import com.itgarden.product.entity.*;
import com.itgarden.product.exception.InvalidInputException;
import com.itgarden.product.exception.ResourceNotFoundException;
import com.itgarden.product.mapper.ProductItemMapper;
import com.itgarden.product.mapper.ProductMapper;
import com.itgarden.product.messages.ResponseMessage;
import com.itgarden.product.repository.*;
...
//    private VendorRepository vendorRepository;
    ...
    public ResponseMessage save(ProductInfo productInfo) throws Exception {
        ...
        // Web Service should give vendor
        List<Vendor> vendorRequest = null; //product.getVendors();
        ...
        if (StringUtils.isEmpty(product.getProductCode())) {
            purchaseOrder = purchaseOrderRepository.findPurchaseOrderByVendorIdAndProductNameAndPurchaseOrderStatus(
                    vendorRequest.get(0).getId(), product.getName(), PurchaseOrderStatus.READY);
...
//            product.setVendors(vendors);
    ...
    public ResponseMessage update(ProductInfo productInfo) {
        ResponseMessage responseMessage = null;
        Product product = ProductMapper.INSTANCE.productInfoToProduct(productInfo);
        Vendor vendor = null; //product.getVendors().get(0);
...
//                product.getVendors().get(0), product.getName(), PurchaseOrderStatus.READY);
        PurchaseOrder purchaseOrder = purchaseOrderRepository.findPurchaseOrderByVendorIdAndProductNameAndPurchaseOrderStatus(
                0l, product.getName(), PurchaseOrderStatus.READY);
